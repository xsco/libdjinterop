...
{
void schema_1_13_2::verify_list(sqlite::database& db) const
{
    // Schema 1.13.2 does not have the lack of explicit types on some columns
    // that was present in 1.13.1, which may be down to a difference between
    // the SC5000 firmware and the Engine Prime software application.  The
    // schema is unchanged vs. 1.13.0.
    schema_1_13_0::verify_list(db);
}

void schema_1_13_2::verify_performance_data(sqlite::database& db) const
{
    // Schema 1.13.2 does not have the lack of explicit types on some columns
    // that was present in 1.13.1, which may be down to a difference between
    // the SC5000 firmware and the Engine Prime software application.  The
    // schema is unchanged vs. 1.11.1.
    schema_1_11_1::verify_performance_data(db);
}

void schema_1_13_2::create_music_schema(sqlite::database& db)
{
    ...
    db << "INSERT INTO music.Information ([uuid], [schemaVersionMajor], "
          ...
          "VALUES (?, ?, ?, ?, ?, ?)"
       << uuid_str << version_1_13_2.maj << version_1_13_2.min
       << version_1_13_2.pat << current_played_indicator_fake_value << 0;
...
void schema_1_13_2::create_performance_schema(sqlite::database& db)
{
