...
{
void schema_1_13_0::verify_music_master_list(sqlite::database& db) const
{
    {
        ...
        ++iter;
        validate(
            iter, end, "music", "table", "InternalDatabase",
            "InternalDatabase");
        ++iter;
...
void schema_1_13_0::verify_internal_database(sqlite::database& db) const
{
    {
        table_info cols{db, "music", "InternalDatabase"};
        auto iter = cols.begin(), end = cols.end();
        validate(iter, end, "name", "TEXT", 0, "", 0);
        ++iter;
    ...
    }
    {
        ...
        validate(iter, end, "index_InternalDatabase_uuid", 0, "c", 0);
        ++iter;
...
void schema_1_13_0::verify_list(sqlite::database& db) const
{
    {
...
void schema_1_13_0::verify_music_schema(sqlite::database& db) const
{
    // New `InternalDatabase` table.
    verify_music_master_list(db);

    // List now has an `ordering` column.
...
void schema_1_13_0::create_music_schema(sqlite::database& db)
{
    ...
    db << "CREATE TABLE music.List ( [id] INTEGER, [type] INTEGER, [title] "
          ...
          "[ordering] INTEGER, PRIMARY KEY ( [id], [type] ) );";
    ...
    db << "INSERT INTO music.Information ([uuid], [schemaVersionMajor], "
          ...
          "VALUES (?, ?, ?, ?, ?, ?)"
       << uuid_str << version_1_13_0.maj << version_1_13_0.min
       << version_1_13_0.pat << current_played_indicator_fake_value << 0;
...
void schema_1_13_0::create_performance_schema(sqlite::database& db)
{
