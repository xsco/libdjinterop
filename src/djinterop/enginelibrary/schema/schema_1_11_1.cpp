...
void schema_1_11_1::verify_performance_data(sqlite::database& db) const
{
    {
        ...
        ++iter;
        validate(iter, end, "hasTraktorValues", "NUMERIC", 0, "", 0);
        ++iter;
...
void schema_1_11_1::verify_music_schema(sqlite::database& db) const
{
    // List of tables/views unchanged since 1.9.1.
    verify_music_master_list(db);

    // Lists now have a track count.
    // Track has a UNIQUE index constraint on `path`.
...
void schema_1_11_1::verify_performance_schema(sqlite::database& db) const
{
    // List of tables unchanged since 1.6.0.
    verify_performance_master_list(db);

    // Version 1.11.1 adds support for Traktor-imported tracks.
    verify_information(db, "perfdata");
    verify_performance_data(db);
}

void schema_1_11_1::create_music_schema(sqlite::database& db)
{
    ...
    db << "INSERT INTO music.Information ([uuid], [schemaVersionMajor], "
          ...
          "VALUES (?, ?, ?, ?, ?, ?)"
       << uuid_str << version_1_11_1.maj << version_1_11_1.min
       << version_1_11_1.pat << current_played_indicator_fake_value << 0;
...
void schema_1_11_1::create_performance_schema(sqlite::database& db)
{
