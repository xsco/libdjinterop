...
{
void schema_2_21_1::verify_track(sqlite::database& db) const
{
    // Schema 2.21.1 tweaks the `index_Track_bpmAnalyzed` index.
    {
...
void schema_2_21_1::create(sqlite::database& db)
{
    ...
    db << "CREATE TABLE Track ( 	id INTEGER PRIMARY KEY AUTOINCREMENT, 	"
          ...
          "originDatabaseUuid TEXT, 	originTrackId INTEGER, 	trackData "
          "BLOB, 	overviewWaveFormData BLOB, 	beatData BLOB, 	quickCues "
          "BLOB, 	loops BLOB, 	thirdPartySourceId INTEGER, 	"
          "streamingFlags INTEGER, 	explicitLyrics BOOLEAN, 	"
          "activeOnLoadLoops INTEGER, 	lastEditTime DATETIME, 	CONSTRAINT "
    ...
    db << "CREATE VIEW PerformanceData AS SELECT 	id AS trackId, 	"
          ...
          "quickCues, 	loops, 	thirdPartySourceId, 	activeOnLoadLoops FROM "
          "Track;";
    db << "CREATE TRIGGER "
    ...
    db << "CREATE TRIGGER trigger_instead_update_loops_PerformanceData INSTEAD "
          ...
          "Track SET 		loops = NEW.loops 	WHERE Track.id = NEW.trackId; "
          "END;";
    ...
    db << "CREATE TRIGGER "
          ...
          "	UPDATE Track SET 		activeOnLoadLoops = NEW.activeOnLoadLoops "
          "	WHERE Track.id = NEW.trackId; END;";
