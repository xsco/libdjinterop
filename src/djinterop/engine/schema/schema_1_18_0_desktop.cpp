...
{
void schema_1_18_0_desktop::verify_track(sqlite::database& db) const
{
    {
        ...
        ++iter;
        validate(iter, end, "isBeatGridLocked", "NUMERIC", 0, "0", 0);
        ++iter;
...
void schema_1_18_0_desktop::verify_performance_data(sqlite::database& db) const
{
...
void schema_1_18_0_desktop::verify_music_schema(sqlite::database& db) const
{
...
void schema_1_18_0_desktop::create_music_schema(sqlite::database& db)
{
    ...
    db << "CREATE TABLE music.List (id INTEGER, type INTEGER, title TEXT, path "
          "TEXT, isFolder NUMERIC, trackCount INTEGER, 		ordering INTEGER, "
          "isExplicitlyExported NUMERIC DEFAULT (1), PRIMARY KEY (id, type));";
    db << "CREATE TABLE IF NOT EXISTS music.\"Information\" ( [id] INTEGER "
    ...
    db << "CREATE TABLE IF NOT EXISTS music.\"AlbumArt\" ( [id] INTEGER "
          "PRIMARY KEY AUTOINCREMENT, [hash] TEXT, [albumArt] BLOB);";
    ...
    db << "CREATE TABLE music.Track ( [id] INTEGER PRIMARY KEY AUTOINCREMENT, "
          ...
          "[isBeatGridLocked] NUMERIC DEFAULT 0, CONSTRAINT C_path UNIQUE "
...
void schema_1_18_0_desktop::create_performance_schema(sqlite::database& db)
{
    // Note that the below statements create the NUMERIC-typed variant of the
    // 1.18.0 schema.
    ...
    db << "CREATE TRIGGER perfdata.trigger_after_update_PerformanceDataAFTER "
