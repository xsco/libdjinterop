...
#include "schema_3_0_2.hpp"

#include "schema_validate_utils.hpp"

namespace djinterop::engine::schema
{
void schema_3_0_2::verify_track(sqlite::database& db) const
{
    // Schema 3.0.2 adds a new column on Track called albumArtSourceHash.
    {
        ...
        ++iter;
        validate(iter, end, "albumArtSourceHash", "CHAR(40)", 0, "", 0);
        ++iter;
...
void schema_3_0_2::create(sqlite::database& db)
{
    ...
    db << "CREATE TABLE Track ( 	id INTEGER PRIMARY KEY AUTOINCREMENT, 	"
          ...
          "albumArtSourceHash CHAR(40), 	CONSTRAINT "
    ...
    db << "CREATE TRIGGER trigger_after_update_Track_fix_origin AFTER UPDATE "
          ...
          "		originTrackId = NEW.id, 		originDatabaseUuid = (SELECT "
          "uuid FROM Information) 	WHERE track.id = NEW.id; END;";
    db << "CREATE TRIGGER trigger_after_update_only_Track_timestamp 	AFTER "
          "UPDATE OF	length, bpm, year, filename, bitrate, bpmAnalyzed, "
          "albumArtId, 	title, artist, album, genre, comment, label, composer, "
          "remixer, key, rating, albumArt, 	fileType, isAnalyzed, "
          "isBeatgridLocked, explicitLyrics 	ON Track 	FOR EACH ROW BEGIN "
          "	UPDATE Track SET lastEditTime = strftime('%s') WHERE "
          "ROWID=NEW.ROWID; END;";
    db << "CREATE TRIGGER trigger_after_insert_Track_insert_performance_data "
          "AFTER INSERT ON Track BEGIN 	INSERT INTO PerformanceData(trackId) "
          "VALUES(NEW.id); END;";
    db << "CREATE TRIGGER trigger_PerformanceData_after_update_Track_timestamp "
          ...
          "strftime('%s') 	WHERE id = NEW.trackId; END;";
