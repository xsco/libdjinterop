...
{
void schema_2_20_2::verify_change_log(sqlite::database& db) const
{
    // Schema 2.20.2 adds a new index on ChangeLog for the `trackId` column.
    {
    ...
    }
    {
        ...
        validate(iter, end, "index_ChangeLog_trackId", 0, "c", 0);
        ++iter;
        validate_no_more(iter, end);
    }
    {
        index_info ii{db, "index_ChangeLog_trackId"};
...
void schema_2_20_2::create(sqlite::database& db)
{
    ...
    db << "CREATE INDEX index_ChangeLog_trackId ON ChangeLog(trackId);";
