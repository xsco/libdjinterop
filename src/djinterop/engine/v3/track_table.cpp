...
int64_t track_table::add(const track_row& row)
{
    ...
    if (context_->schema >= engine_schema::schema_3_0_2)
    {
    ...
    if (context_->schema >= engine_schema::schema_3_0_2)
    {
        context_->db
                << "SELECT id, playOrder, length, bpm, year, path, filename, "
                   ...
                   "FROM Track WHERE id = ?"
                << id >>
            [&](int64_t track_id, std::optional<int64_t> play_order,
                int64_t length, std::optional<int64_t> bpm,
                std::optional<int64_t> year, std::string path,
                std::string filename, std::optional<int64_t> bitrate,
                ...
                std::optional<std::string> remixer, std::optional<int32_t> key,
                int64_t rating, std::optional<std::string> album_art,
                ...
                std::vector<std::byte> album_art_source_hash)
        {
            ...
            result = track_row{
                ...
                std::move(album_art_source_hash)};
        };
    }
    else
    {
...
void track_table::set_title(int64_t id, const std::optional<std::string>& title)
{
...
void track_table::set_album(int64_t id, const std::optional<std::string>& album)
{
...
void track_table::set_genre(int64_t id, const std::optional<std::string>& genre)
{
...
void track_table::set_label(int64_t id, const std::optional<std::string>& label)
{
...
{
    return get_column<std::optional<std::string>>(context_->db, id, "composer");
}
...
{
    return get_column<std::optional<std::string>>(context_->db, id, "albumArt");
}
...
std::vector<std::byte> track_table::get_album_art_source_hash(int64_t id)
{
    if (context_->schema < engine_schema::schema_3_0_2)
        throw djinterop::unsupported_operation{
            "The `albumArtSourceHash` column is not available for this "
            "database version"};

    return get_column<std::vector<std::byte>>(
        context_->db, id, "albumArtSourceHash");
}

void track_table::set_album_art_source_hash(
    int64_t id, const std::vector<std::byte>& album_art_source_hash)
{
    ...
    set_column<std::vector<std::byte>>(
        context_->db, id, "albumArtSourceHash", album_art_source_hash);
}
