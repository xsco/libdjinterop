...
///
/// The format has one tree of playlists and no concept of a crate, so
/// a crate here is the same row as the playlist of the same identifier. The
/// database reports `playlists_and_crates_are_distinct=false`.
class crate_impl : public djinterop::crate_impl
{
public:
    crate_impl(std::shared_ptr<onelibrary_context> context, int64_t id);
