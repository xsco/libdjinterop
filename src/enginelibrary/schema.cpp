...
{
    // TODO - add support for DB schema version 1.9.1 (new "List" tables, old ones as VIEWs)
    return (
        version == version_1_6_0 ||
        version == version_1_7_1);
}
...
schema_version verify_music_schema(sqlite::database &db)
{
    ...
    dispatch(version,
            [&db] { verify_music_schema_1_6_0(db); },
...
schema_version verify_performance_schema(sqlite::database &db)
{
    auto version = get_version(db);
    dispatch(version,
            [&db] { verify_performance_schema_1_6_0(db); },
...
{
    dispatch(version,
            [&db] { create_music_schema_1_6_0(db); },
...
{
    dispatch(version,
            [&db] { create_performance_schema_1_6_0(db); },
